...
import { defaultRateLimit, normalizeText, parseCustomerRecord } from "./customer-schema.js";
...
function createCustomerRegistry({ configPath, customers = [] } = {}) {
  ...
  let records = customers.map((customer) => parseCustomerRecord(customer));
  ...
  function loadFromDisk() {
    ...
    records = parsed.map((customer) => parseCustomerRecord(customer));
    return records;
  }
  ...
  function upsertCustomer(input = {}) {
    ...
    const nextRecord = parseCustomerRecord(
      {
        ...input,
        customerId,
      ...
      {
        existing: existing || { rateLimit: defaultRateLimit, allowedTools: ["*"], expiresAt: null },
        errorFactory: (message) => registryError("invalid_customer", message)
      }
    );
  ...
  function rotateCustomerToken(customerId) {
    ...
    const rawToken = createToken();
    const next = parseCustomerRecord(
      {
        ...existing,
